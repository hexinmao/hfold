...
bool validateStructure(char* structure, char* sequence){
    ...
    for(int i=0; i<strlen(structure);i++) {
        ...
        if(curr == ')' || curr == '}' || curr == ']') { //is close bracket
            ...
            if(closeBracketArray.find_first_of(curr) != openBracketArray.find_first_of(mystack.top())){ //if current bracket is not corresponding bracket of what we popped
